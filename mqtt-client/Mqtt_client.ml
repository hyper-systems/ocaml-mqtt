...
let read_packet inch =
  ...
  let (msgid, opts) = Mqtt_packet.Decoder.decode_fixed_header (Char.code header_byte) in
  ...
  let pkt =
    ...
    |> Mqtt_packet.Decoder.decode_packet opts msgid in
  Lwt.return (opts, pkt)
...
module Client = struct
  ...
  let default_error_fn _client exn =
    let%lwt () = Log.err (fun log -> log "Mqtt_client: unhandled client error") in
    Lwt.fail exn
  ...
  let connect
    ...
    let connect_packet = Mqtt_packet.Encoder.connect
        ?credentials:opt.cxn_data.credentials
    ...
    | (_, Connack pkt) ->
      Lwt.fail (Connection_error (Mqtt_packet.connection_status_to_string pkt.connection_status))

    | _ -> Lwt.fail (Connection_error ("Invalid response from broker on connection"))


  let publish
      ?(dup=false) ?(qos=Atleast_once) ?(retain=false)
    ...
    let pkt_data =
      Mqtt_packet.Encoder.publish ~dup ~qos ~retain ~id ~topic payload in
    Lwt_io.write oc pkt_data >>= fun () ->
    Lwt_condition.wait cond


  let subscribe ?(dup=false) ?(qos=Atleast_once) ?(retain=false) ?id topics client =
    let (_, oc) = client.cxn in
    let subscribe_packet =
      Mqtt_packet.Encoder.subscribe ~dup ~qos ~retain ?id topics in
